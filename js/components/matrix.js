...
export function showDetailedTooltip(gunasthanId, thanaIndex) {
    ...
    // Inject "मुझे समझाओ" block when active
    try {
        const explainMode = (typeof window !== 'undefined' && window.__getMatrixMode)
            ? window.__getMatrixMode() : 'numbers';
        if (explainMode === 'explain' && window.__generateExplanation) {
            const explain = window.__generateExplanation(gunasthanId, thanaIndex);
            if (explain && explain.prose) {
                bodyHtml += `
                    <div class="tooltip-explain-block">
                        <div class="tooltip-explain-title">💡 ${explain.title}</div>
                        <div class="tooltip-explain-prose">${explain.prose}</div>
                        <div style="margin-top: 12px;">
                            <button class="speak-btn" data-speaks-id="cell-${gunasthanId}-${thanaIndex}"
                                    onclick="event.stopPropagation(); window.__speakCellExplain(${gunasthanId}, ${thanaIndex});">
